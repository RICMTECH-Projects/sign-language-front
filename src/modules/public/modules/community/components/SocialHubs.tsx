...
import { useSocialHubsAnimation } from '../hooks/useSocialHubsAnimation';
import { SOCIAL_HUBS_DATA } from '../constants/social-hubs.const';
...
export function SocialHubs() {
	const container = useSocialHubsAnimation();
			...
			<div className='max-w-7xl mx-auto'>
				<div className='text-center mb-16 space-y-4'>
					<h2 className='text-3xl md:text-5xl font-serif text-foreground'>
						Escoge donde conectar
					</h2>
					<p className='text-foreground/60 max-w-xl mx-auto text-lg leading-relaxed'>
						Diferentes espacios para diferentes necesidades. Únete al que mejor
						se adapte a tu ritmo.
					</p>
				</div>

				<div className='grid grid-cols-1 md:grid-cols-3 gap-8'>
					{SOCIAL_HUBS_DATA.map((hub) => {
						const IconComponent = ICON_MAP[hub.icon as keyof typeof ICON_MAP];
						return (
							<a
								key={hub.id}
								href={hub.link}
								...
								<div
									className={`w-16 h-16 rounded-2xl ${hub.color} text-white flex items-center justify-center shadow-lg transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3`}>
